...
new #[Layout('layouts.guest')] class extends Component
{
    ...
    public function login(): void
    {
        ...
        try {
            ...
            $this->dispatch('two-factor-required');
            ...
            $this->addError('form.email', __('An unexpected error occurred. Please try again.'));
        }
    }
    ...
    {
        $this->validate([
            'form.two_factor_code' => ['required', 'string'],
        ]);

        try {
            if (Auth2FA::input($this->form->two_factor_code)) {
                Session::regenerate();
                $this->redirectIntended(default: route('overview', absolute: false), navigate: true);
            }
        } catch (ValidationException $e) {
            $this->addError('form.two_factor_code', $e->getMessage());
        }
    }
}
?>
...
<div>
    ...
    @if ($showTwoFactorForm)
        <form wire:submit="confirmTwoFactor" class="mt-8 space-y-6">
            <div>
                <x-input-label for="two_factor_code" :value="__('Two-Factor Authentication Code')" class="block text-sm font-medium text-gray-700 dark:text-gray-300"/>
                <x-text-input autofocus wire:model="form.two_factor_code" id="two_factor_code" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white sm:text-sm" type="text" name="two_factor_code" inputmode="numeric" pattern="[0-9]*" autocomplete="one-time-code" required autofocus/>
                <x-input-error :messages="$errors->get('form.two_factor_code')" class="mt-2"/>
            </div>
