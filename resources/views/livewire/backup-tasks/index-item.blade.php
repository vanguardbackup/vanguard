        ...
        <div class="justify-start mx-4 space-x-5 hidden md:flex">
            <div>
                @if ($backupTask->isFilesType())
                    <div id="files-type" class="rounded-full bg-purple-50 dark:bg-purple-950 border border-purple-100 dark:border-purple-900 p-2 overflow-hidden" title="{{ __('Files Task') }}">
                        @svg('heroicon-o-document-duplicate', 'h-5 w-5 text-purple-600 dark:text-purple-400')
                    </div>
                @elseif ($backupTask->isDatabaseType())
                    <div id="database-type" class="rounded-full bg-cyan-50 dark:bg-cyan-950 border border-cyan-100 dark:border-cyan-900 p-2 overflow-hidden" title="{{ __('Database Task') }}">
