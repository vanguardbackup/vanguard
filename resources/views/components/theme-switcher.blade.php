<div x-data="setupThemeSwitcher()" x-init="init()">
    <x-modal name="theme-switcher" :focusable="true" maxWidth="2xl">
        ...
        <div class="transform transition-all sm:max-w-2xl sm:w-full">
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <template x-for="(option, index) in [
                    { value: 'light', label: 'Light', icon: 'sun', description: 'Bright and clear for daytime use.', color: 'yellow' },
                    { value: 'dark', label: 'Dark', icon: 'moon', description: 'Easy on the eyes in low-light environments.', color: 'indigo' },
                    { value: 'system', label: 'System', icon: 'computer-desktop', description: 'Automatically matches your device settings.', color: 'green' }
                ]" :key="index">
                    <button
                        @click="setTheme(option.value)"
                        class="relative flex flex-col items-center w-full px-4 py-5 text-center rounded-lg transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 dark:focus:ring-primary-400 border group overflow-hidden"
                        :class="{
                    ...
                    >
                        <div class="absolute inset-0 opacity-0 group-hover:opacity-10" :class="`bg-${option.color}-500`"></div>
                        <svg class="w-12 h-12 mb-4 transition-transform duration-300 ease-in-out group-hover:scale-110" :class="{
                        ...
                        'text-emerald-600 dark:text-emerald-400': option.value === 'system'}"
                            ...
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" x-bind:d="getIconPath(option.icon)" />
                        </svg>
                        <span x-text="option.label" class="text-lg font-semibold mb-2"></span>
                        <p class="text-sm" x-text="option.description"></p>
                        <div x-show="theme === option.value" class="absolute top-2 right-2 p-1 rounded-full" :class="`bg-${option.color}-100 dark:bg-${option.color}-900`">
                            <svg class="w-5 h-5" :class="`text-${option.color}-500 dark:text-${option.color}-400`" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="mt-8 text-center">
                <x-secondary-button centered @click="$dispatch('close')" class="px-6 py-2 transition-all duration-300 ease-in-out hover:bg-gray-100 dark:hover:bg-gray-700">
                    {{ __('Close') }}
                </x-secondary-button>
            </div>
        </div>
        ...
        return {
            ...
            getIconPath(icon) {
                const paths = {
                    ...
                    'computer-desktop': 'M20 14.5V6.5C20 4.61438 20 3.67157 19.4142 3.08579C18.8284 2.5 17.8856 2.5 16 2.5H8C6.11438 2.5 5.17157 2.5 4.58579 3.08579C4 3.67157 4 4.61438 4 6.5V14.5 M12 5.5H12.009 M3.49762 15.5154L4.01953 14.5H19.9518L20.5023 15.5154C21.9452 18.177 22.3046 19.5077 21.7561 20.5039C21.2077 21.5 19.7536 21.5 16.8454 21.5L7.15462 21.5C4.24642 21.5 2.79231 21.5 2.24387 20.5039C1.69543 19.5077 2.05474 18.177 3.49762 15.5154Z'
                };
                return paths[icon] || '';
            }
        }
