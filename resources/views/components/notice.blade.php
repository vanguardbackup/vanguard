...
>
    <div class="flex items-start space-x-3">
        <div class="flex-shrink-0">
            <svg class="h-5 w-5 {{ $selectedColors['icon'] }}" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
        ...
        <div class="flex-1 min-w-0">
            @if ($title)
                <h3 class="text-sm font-medium {{ $selectedColors['text'] }}">
                    {{ $title }}
                </h3>
            @endif
            <div class="mt-1 text-sm {{ $selectedColors['body'] }}">
                @if ($text)
                    {{ $text }}
                @else
                    {{ $slot }}
